...
voxelizer::scene_renderer::scene_renderer()
{
	...
	vert_shader.source_from_string(shinji::load_resource_from_bundle("resources/shaders/scene.vert").first);
	vert_shader.compile();

	m_program.attach_shader(vert_shader);

	Shader frag_shader(GL_FRAGMENT_SHADER);
	frag_shader.source_from_string(shinji::load_resource_from_bundle("resources/shaders/scene.frag").first);
	frag_shader.compile();
